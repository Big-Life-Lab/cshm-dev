...
**Sex stratification.** Table 1 is stratified by `survey_var(cfg, "sex")` (mapped to `DHH_SEX`). This is consistent with the APC models, which are fit separately by sex. The config abstraction means the table code does not contain any hardcoded CCHS variable names.
