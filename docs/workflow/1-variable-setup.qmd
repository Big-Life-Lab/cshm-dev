...
Before any data can be loaded, the pipeline needs to know *which* variables to extract from the CCHS and *how* to recode them across survey cycles. This is defined entirely in two plain-text worksheets — no variable names or categories are hard-coded in R. Keeping the variable definition separate from code means you can adapt the pipeline to a new survey or country by editing the worksheets, not the R code.

This page serves as the **study data dictionary** — a complete description of every variable used in the CSHM, generated directly from those worksheets. If the worksheets change, this page updates automatically on re-render.
...
| `~/github/cchsflow/inst/extdata/variable_details.csv` | cchsflow base recoding rules (cycles 2001–2018) |
| `worksheets/cshm-variable-details.csv` | CSHM extension rows (2019–20 and 2022 cycles) |
...
master_cycle_map <- c(
  ...
  cchs2019_2020_m = "2019-20", cchs2022_m = "2022", cchs2023_m = "2023"
)
...
get_cycle_coverage_vec <- function(var_name, variable_details_sheet) {
  ...
  if (nrow(rows) == 0) {
    ...
    return(list(pumf = pumf_v, master = master_v))
  }
  ...
  master_yrs <- unname(master_cycle_map[all_datasets[all_datasets %in% names(master_cycle_map)]])
...
```{r data-dictionary}
#| results: asis
design_vars <- c("SurveyCycle", "WTS_M")
...
cov_rows <- variables_sheet |>
  ...
  mutate(
    ...
    cov = list(get_cycle_coverage_vec(variable, variable_details_sheet))
  ) |>
  ungroup()
...
**Extension rows.** cchsflow v3 `variable_details.csv` covers CCHS demographics only through 2017–18. The CSHM extension rows (`worksheets/cshm-variable-details.csv`) add recoding rules for `DHH_SEX`, `DHHGAGE_cont`, `GEOGPRV`, and `WTS_M` for the 2019–20 and 2022 cycles. As cchsflow v3 extends its coverage, these extension rows can be removed.
