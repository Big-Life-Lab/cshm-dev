---
title: "Reproducing the Manuel et al. (2020) Ontario analysis"
subtitle: "Replicating the original Ontario smoking histories model in CSHM"
...
This guide describes how to reproduce the Ontario smoking histories analysis of Manuel et al. (2020) using the CSHM pipeline. The original study — "Smoking patterns based on birth-cohort-specific histories from 1965 to 2013, with projections to 2041" (*Health Reports*, 2020) — estimated smoking initiation and cessation probabilities using Ontario CCHS respondents with an Age-Period-Cohort model in SAS.

The CSHM re-implements this analysis in R, extends it to all Canadian provinces, and updates the CCHS cycles from 2013–14 to 2022.

See also: [docs/reproducing-Ontario-study.qmd](reproducing-ontario-study.qmd) for a detailed variable-by-variable mapping from the original SAS code.

## Purpose of this guide

1. Document how the Manuel et al. (2020) model maps onto CSHM pipeline stages
2. Show which config parameters reproduce the Ontario-only, 2013-restricted analysis
3. Identify where the CSHM extends or departs from the original
4. Provide validation checks against published Ontario estimates

## Original study overview

Manuel et al. (2020) used CCHS cycles 2001–2013 to:

- Reconstruct birth-cohort-specific smoking histories for Ontario
- Estimate smoking initiation and cessation probabilities by age, period, and cohort
- Project prevalence forward to 2041
- Apply the Holford et al. (2014) APC framework with constrained natural splines
