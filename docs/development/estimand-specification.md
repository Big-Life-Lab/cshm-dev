...
- **Initiation risk.** From the initiation floor (`initiation_floor(cfg)`; PUMF 13, Master 8) to the age at first cigarette (event) or the survey age (censored), whichever comes first. Never smokers are at risk at every age up to the survey.
- **Cessation risk.** From the person's own age at first cigarette to the age they stopped completely (event) or the survey age (censored). No person-year before entry. There is no fixed minimum age; the entry age must fall within the worksheet range for `age_first_cigarette`, and a respondent whose entry age falls outside it is excluded and counted (`excluded_entry_invalid`).
