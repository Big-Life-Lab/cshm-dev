...
version-history:
  - version: "0.4.0"
    date: "2026-08-07"
    description: "Methodological amendments following the external methodological review and team adjudication (2026-08-07). New §3.4.1 declares the smoking states and transition estimands: the established-smoking model of Holford et al. (2014) as replicated for the CCHS by Manuel et al. (2020) — never/current/former states, entry at first whole cigarette, cessation defined as stopping smoking completely, daily onset and intensity as attributes of the current state; single-spell event-time conventions (per-person cessation clock from state entry, two-year durable-cessation rule, same-age ordering to be finalized with a prespecified sensitivity). APC identification restated as an explicit estimable-functions parameterization with a declared linear-drift allocation, stored training-to-prediction basis, and per-fit rank/estimability diagnostics, distinguished from the extrapolation constraints (Holford 2006). Uncertainty analysis specified: CCHS bootstrap replicate weights primary in the Master-file analysis (respondent-level blocks, per-imputation combination validated by simulation, propagation to rate tables); person-level bootstrap for PUMF analyses labelled approximate; simulated replicate weights (MockData) verify code only. Mortality adjustment: MPoRT remains primary conditional on life-table calibration and transportability validation; the estimator's entry point (pre-fit weights vs post-fit correction) to be derived explicitly before implementation; uncorrected outputs labelled survivor-conditioned. Imputation extended to universe-nested chained equations with in-iteration ordering constraints, all completed datasets analysed, and m selected by Monte Carlo stability (floor m = 5; Appendix D). Internal validation strengthened (held-out forms, recall-consistency checks); external validation formally scoped out with rationale (comparator surveys' own measurement and response error), with the CHMS biomarker adjustment route foreshadowed. Subsections of §3.4 renumbered to accommodate §3.4.1."
  - version: "0.3.2"
    date: "2026-06-10"
...
The model estimates transitions between three mutually exclusive smoking states — **never**, **current**, and **former** — following the established-smoking framework of Holford et al. (2014) [@Holford_AJPM_2014], which has been applied across several jurisdictions and which Manuel et al. (2020) [@Manuel_HR_2020] replicated for Ontario using the CCHS. This study follows the Manuel et al. implementation as its operative precedent because it uses the same survey instrument. Entry into the current state (initiation) occurs at the age of the first whole cigarette among respondents meeting the ever-smoker definition. Exit from the current state (cessation) occurs on stopping smoking completely — not on stopping daily smoking. Daily-smoking onset and smoking intensity (cigarettes per day) are modelled as attributes of the current state (§3.4.4), not as separate primary transitions; occasional smokers who formerly smoked daily remain in the current state until they stop smoking entirely.

Event-time conventions: the model is a single-spell, discrete-time construction, and each person's cessation risk clock starts at their own state-entry age. The primary cessation definition is the most recent cessation sustained for at least two years, with more recent quitters censored at the reported quit age, consistent with the Holford and Manuel implementations. Where initiation and cessation are reported at the same integer age, the ordering convention will be finalized in a documented analysis specification before model fitting, with a prespecified sensitivity analysis excluding or interval-censoring same-age transitions. Relapse histories are not identifiable from the CCHS's single-event retrospective capture; their influence is addressed in sensitivity analyses (§3.5).

The target population is the Canadian household population as covered by the CCHS. For immigrants, person-years before immigration are excluded from smoking-history reconstruction (§3.3), so risk sets begin no earlier than entry into the Canadian population. Respondents whose state-entry age is missing or implausible enter the multiple-imputation procedure (§3.4.2 and Appendix D) rather than being excluded or reclassified.
...
2.  **Imputation:** Item non-response (don't know/refused; tagged NA(b)) on smoking status, initiation age, and cessation timing will be addressed using Multiple Imputation by Chained Equations (MICE) with at least *m* = 5 imputations; the production number of imputations is selected from Monte Carlo stability of the final rate-table outputs. Imputation is restricted to item non-response; structural missingness — not-applicable (NA(a), e.g., initiation age for never-smokers) and not-asked-this-cycle (NA(c)) — is preserved and never imputed directly. Imputation is **universe-nested**: where smoking-state membership is itself affected by item non-response, state-gate variables are imputed jointly with their nested history variables — gates first, nested event times conditionally within the currently imputed universe — and logical ordering constraints (first cigarette no later than cessation; cessation no later than survey age) are enforced during the chained-equations iterations rather than repaired afterward. The imputation model includes the survey design variables (cycle, sampling weight) and sociodemographic predictors (age, sex, province, education) alongside the smoking history variables, preserving congeniality with the APC analysis models, plus auxiliary covariates (marital status, alcohol use, body mass index, self-rated general and mental health, life stress, community belonging, chronic conditions, and physical activity) that improve imputation quality and describe the study base for planned related studies. Derived variables (e.g., pack-years) are recomputed from their imputed feeder variables rather than imputed directly. Every completed dataset is carried through person-year expansion and model fitting, with estimates pooled across imputations (§3.5). The full specification, including the diagnostic and sensitivity-analysis plan, is given in [Appendix D: Missing data and imputation plan](appendix-imputation.qmd).
...
All models use natural cubic splines to decompose temporal trends. The fundamental APC identity `period = age + cohort` makes the three linear trends jointly non-identifiable, and we treat identification as an explicit estimable-functions problem [@Holford_SM_2006]: the model is parameterized in identifiable components — age shape, period curvature, cohort curvature, and net drift — with the allocation of the non-identifiable linear drift declared explicitly as a scientific assumption rather than left to software behaviour. Sensitivity of the published rate tables to at least one materially different drift allocation will be reported as model-specification uncertainty.

Identifying constraints are distinguished from extrapolation constraints. For projection beyond the observed range, period effects are held constant at the most recent observed values (for initiation and cessation alike) and cohort effects are held constant before 1920 and from 1985 forward, consistent with Manuel et al. (2020) — but these boundary constraints serve extrapolation and do not substitute for the identifying parameterization. The spline bases — knots, boundary knots, and centring — are part of the declared parameterization: the basis constructed at fitting is stored and reused for all prediction and projection, and each fitted model's rank and estimable space are verified diagnostically before use, with extrapolated cells documented separately from cells supported by observations.
...
The precise estimator will be derived and documented explicitly before implementation, including where the correction enters the estimation — adjustment of respondent weights before APC fitting (as described above) or correction of fitted transition probabilities (as in the Ontario implementation's original code); the two constructions are not automatically equivalent, and the chosen estimator must be shown to apply the correction exactly once. The derivation is conditional on validation: calibration of predicted aggregate mortality against Canadian life tables by age, sex, and calendar year; stability diagnostics on the correction factors with prespecified truncation limits; and recovery of known transition rates in synthetic cohorts with smoking-dependent mortality (§3.5). MPoRT remains the primary method conditional on these transportability checks [@Manuel_HR_2020]. Until a correction is implemented, uncorrected outputs are labelled as survivor-conditioned estimates and are not presented as birth-cohort smoking histories.
...
1.  **Internal validation:** Comparing model-reconstructed prevalence against observed CCHS prevalence, including held-out forms — cycles or subsamples excluded from fitting — so the comparison is out-of-fit, and recall-consistency checks in which the retrospective histories reported by the same birth cohort at different survey cycles are compared for coherence.

> ISPOR-SMDM: Internal validation; STRESS: Validation - internal

2.  **External validation:** None in this version, by design. Candidate comparator surveys carry their own measurement and response error structures — CCHS smoking prevalence typically exceeds that of tobacco-specific surveys, reflecting the CCHS's broader sampling frame and higher response rate rather than CCHS error — so cross-source comparison without an explicit error model for both sources would mislead more readily than validate. These measurement and response errors are too often glossed over; we prefer to defer the comparison rather than perform it uninterpretably. A future version may use the Canadian Health Measures Survey's biomarker-validated measurement error (item 4) to *adjust* estimates rather than merely compare them; within-CCHS validation cannot detect a CCHS-wide frame or measurement bias, a limitation we accept and state.

> ISPOR-SMDM: External validation; STRESS: Validation - external

3.  **Uncertainty analysis:** In the Master-file analysis, parameter uncertainty is quantified using the official CCHS bootstrap replicate weights [@StatCan_Bootstrap_2010], applied at the respondent level with each respondent's full reconstructed history entering every replicate as a block. Replicate estimation is repeated within each completed imputation, and design-based within-imputation covariance is combined with between-imputation covariance using a prespecified combining rule validated by simulation. Uncertainty is propagated through to the published rate tables and projections, not reported only on model coefficients. In PUMF-based analyses, where replicate weights and design variables are unavailable, a person-level bootstrap approximates the design as closely as the file permits, supplemented by design-effect checks against Statistics Canada's published approximate coefficients of variation for descriptive estimates; all PUMF intervals are labelled approximate, and definitive intervals come from the Master-file analysis. Simulated replicate weights (generated with the public MockData package) are used to verify the replicate-weight code path in the public repository; they verify code, not inference, and never produce reported estimates.
