...
The model estimates transitions between three mutually exclusive smoking states — **never**, **current**, and **former** — following the established-smoking framework of Holford et al. (2014) [@Holford_AJPM_2014], which has been applied across several jurisdictions, and Manuel et al. (2020) [@Manuel_HR_2020] for Ontario using the CCHS. This study follows the Manuel et al. implementation because it uses the same survey instrument. Entry into the current state (initiation) occurs at the age of the first whole cigarette among respondents meeting the ever-smoker definition. Exit from the current state (cessation) occurs on stopping smoking completely — not on stopping daily smoking. Daily-smoking onset and smoking intensity (cigarettes per day) are modelled as attributes of the current state (§3.4.4), not as separate primary transitions; occasional smokers who formerly smoked daily remain in the current state until they stop smoking entirely.

Event-time conventions: the model is a single, discrete-time construction, and each person's cessation risk starts at their own entry age. The primary cessation definition is the most recent cessation sustained for at least two years, with more recent quitters censored at the reported quit age, consistent with the Holford and Manuel implementations. Where initiation and cessation are reported at the same integer age, the ordering convention will be finalized in a documented analysis specification before model fitting, with a prespecified sensitivity analysis excluding or interval-censoring same-age transitions. Relapse histories are not identifiable from the CCHS's single-event retrospective capture; their influence is addressed in sensitivity analyses (§3.5).
